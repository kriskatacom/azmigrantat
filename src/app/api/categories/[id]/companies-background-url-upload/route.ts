...
export async function DELETE(req: Request, { params }: Params) {
    ...
    try {
        // 1️⃣ Взимаме категорията с текущото companies_background_url
        ...
        if (city.companies_background_url) {
            deleteUploadedFile(city.companies_background_url);
        }

        // 3️⃣ Нулираме companies_background_url в базата
        const categoryUpdated = await updateCategory(Number(id), {
            companies_background_url: null,
        });
