...
export default function NewTaxiForm({ taxi }: Params) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post("/api/taxis", formData);

            if (res.data.success) {
                toast.success("Таксиметровата компания е запазена!");

                if (res.status === 201)
                    router.push(`/admin/taxis/${res.data.taxiId}`);
                if (res.status === 200) router.push("/admin/taxis");
            }
        } catch (err: any) {
