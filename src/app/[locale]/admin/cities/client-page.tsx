...
import { columns } from "@/app/[locale]/admin/cities/columns";
...
export default function ClientPage({ data }: ClientPageProps) {
    ...
    async function onBulkDelete(selectedIds: (string | number)[]) {
        try {
            const res = await axios.post("/api/cities/bulk-delete", {
                ids: selectedIds,
            });

            if (res.status === 200) {
                toast.success(
                    `Бяха премахнати ${res.data.deletedCount} градове.`,
                );
                router.refresh();
            }
        } catch (error) {
    ...
    const handleReorder = async (reorderedData: City[]) => {
        try {
