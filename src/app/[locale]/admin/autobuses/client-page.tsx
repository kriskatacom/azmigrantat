...
import { columns } from "@/app/[locale]/admin/autobuses/columns";
...
export default function ClientPage({ data }: ClientPageProps) {
    ...
    async function onBulkDelete(selectedIds: (string | number)[]) {
        try {
            const res = await axios.post("/api/autobuses/bulk-delete", {
                ids: selectedIds,
            });

            if (res.status === 200) {
                toast.success(
                    `Бяха премахнати ${res.data.deletedCount} автобусни гари.`,
                );
                router.refresh();
            }
        } catch (error) {
    ...
    const handleReorder = async (reorderedData: Autobus[]) => {
        try {
