...
import { columns, EmbassyWithCountry } from "@/app/[locale]/admin/embassies/columns";
...
export default function ClientPage({ data }: ClientPageProps) {
    ...
    async function onBulkDelete(selectedIds: (string | number)[]) {
        try {
            const res = await axios.post("/api/embassies/bulk-delete", {
                ids: selectedIds,
            });

            if (res.status === 200) {
                toast.success(
                    `Бяха премахнати ${res.data.deletedCount} посолства.`,
                );
                router.refresh();
            }
        } catch (error) {
    ...
    const handleReorder = async (reorderedData: Embassy[]) => {
        try {
