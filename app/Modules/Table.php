...
class Table
{
    ...
    ?>
        <div class="mb-5 flex items-start gap-5">
            <div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 <?= $style['bg'] ?> <?= $style['text'] ?>">
        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
            <form action="<?= $config['base_url'] ?>" method="GET" class="flex items-end gap-3 flex-1 md:flex-none">
                ...
                <div class="w-full md:w-80">
                    <?php \App\Modules\Form::input('', 'search', $search, 'text', ['placeholder' => 'Търсене...']); ?>
                </div>
                <div class="pb-px">
                    <?php \App\Modules\Form::submit('Търси', 'fa-magnifying-glass'); ?>
                </div>
            </form>
                ...
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        <?php foreach ($columns as $dbField => $label): ?>
                            ...
                            ?>
                            <th class="p-4 font-semibold text-slate-700 <?= empty($dbField) || is_numeric($dbField) ? 'text-right' : '' ?>">
                                <?php if ($isSortable): ?>
                                    <a href="<?= $sortUrl ?>" class="flex items-center gap-1 hover:text-primary transition-colors group">
                                        <?= $label ?>
        ...
        <tr>
            <td colspan="<?= $colspan ?>" class="p-12 text-center">
                <div class="flex flex-col items-center justify-center">
                    <div class="w-16 h-16 bg-slate-50 text-slate-300 rounded-full flex items-center justify-center mb-4 border border-dashed border-slate-200">
