...
class Form
{
    public static function section(string $title, callable $callback, string $icon = '', bool $defaultOpen = true, string $id = '')
    {
        ...
        ?>
        <details class="group bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden mb-2 md:mb-5"
            data-section-key="<?= $sectionKey ?>" id="<?= $id ?>" <?= $isOpen ? 'open' : '' ?>>

            <summary
                class="list-none cursor-pointer md:font-semibold text-lg md:text-xl text-slate-900 border-b border-slate-100 flex items-center justify-between p-2 md:p-5 bg-slate-50/30 hover:bg-slate-50 transition-colors">
                ...
                <i
                    class="fa-solid fa-chevron-down text-sm text-slate-400 transition-transform duration-300 group-open:rotate-180"></i>
            </summary>

            <div class="p-2 md:p-5 space-y-2 md:space-y-5 border-t border-slate-50">
                <?php $callback(); ?>
            </div>
        </details>
    ...
    public static function input(string $label, string $name, string $value = '', string $type = 'text', array $options = [])
    {
        ...
        <div <?= $wrapperId ? "id=\"$wrapperId\"" : "" ?> id="<?= $id ?>">
            ...
            <input type="<?= $type ?>" name="<?= $name ?>" value="<?= htmlspecialchars($value) ?>" <?= $required ?>
    ...
    public static function textarea(string $label, string $name, string $value = '', array $options = [])
    {
        ...
        <div <?= $wrapperId ? "id=\"$wrapperId\"" : "" ?> class="w-full">
            ...
            <textarea id="<?= $id ?>" name="<?= $name ?>" rows="<?= $rows ?>" <?= $required ?> placeholder="<?= $placeholder ?>"
    ...
    public static function select(string $label, string $name, array $choices, $selectedValue = '', array $options = [])
    {
        ...
        <div>
            ...
            <select name="<?= $name ?>" <?= $id ? "id=\"$id\"" : "" ?>
                class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary outline-none transition-all cursor-pointer">
    ...
    public static function image(string $label, string $name, ?string $currentValue = null, array $options = [])
    {
        ...
        <div class="space-y-3 image-upload-container" data-component="image-upload">
            ...
            <div
                class="flex items-center gap-2 md:gap-5 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200 transition-colors hover:bg-gray-50/80">
                <div class="relative shrink-0 group/img">
                    <div
                        class="relative w-40 h-30 md:w-44 md:h-32 rounded-2xl overflow-hidden shadow-sm border-2 border-white bg-white transition-all duration-300 group-hover/img:shadow-xl group-hover/img:-translate-y-1">
                        <img id="preview-<?= $id ?>" src="<?= $fullPath ?>" data-default="<?= $default ?>"
                            class="w-full h-full object-cover transition-transform duration-700 group-hover/img:scale-110">
                        <div id="overlay-<?= $id ?>"
                            class="absolute inset-0 bg-linear-to-t from-black/50 via-transparent to-black/20 opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 <?= !$hasImage ? 'hidden' : '' ?>">
                        </div>
                        <div id="actions-<?= $id ?>"
                            class="absolute top-2 right-2 flex gap-1.5 translate-y-2.5 opacity-0 group-hover/img:translate-y-0 group-hover/img:opacity-100 transition-all duration-300 z-10 <?= !$hasImage ? 'hidden' : '' ?>">
                            <button type="button" id="zoom-btn-<?= $id ?>" data-src="<?= $fullPath ?>"
                            ...
                            <a id="download-btn-<?= $id ?>" href="<?= $fullPath ?>" download
                            ...
                            <button type="button"
                                class="remove-image-btn w-8 h-8 flex items-center justify-center bg-white/90 backdrop-blur-sm text-red-500 rounded-md shadow-md hover:bg-red-500 hover:text-white transition-all duration-200"
                    ...
                    <input type="hidden" name="<?= $name ?>" id="remove-input-<?= $id ?>" value="<?= $currentValue ?? '' ?>">
                </div>

                <div class="flex flex-col gap-2 md:gap-3 flex-1">
                    <label class="cursor-pointer group w-fit"
                        title="<?= $hasImage ? 'Промяна на изображение' : 'Качване на изображение' ?>">
                        <input type="file" id="<?= $id ?>" accept="image/*" class="hidden image-file-input">
                        <div
                            class="flex items-center gap-2 px-3 py-2 md:px-5 md:py-2.5 bg-white border border-gray-200 rounded-md md:rounded-xl shadow-sm text-sm md:font-bold text-gray-700 group-hover:border-indigo-400 group-hover:text-indigo-600 transition-all duration-300">
                            <i class="fa-solid fa-plus upload-icon"></i>
                            <span class="btn-text"><?= $hasImage ? 'Промяна' : 'Качване' ?></span>
                        </div>
                    </label>
                    <div class="md:font-semibold text-xs text-gray-600">JPG, PNG, WebP (Макс. 10MB)</div>
                </div>
            </div>
        </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    document.body.addEventListener('change', function (e) {
                        if (e.target.classList.contains('image-file-input')) {
                            ...
                            if (input.files && input.files[0]) {
                                ...
                                if (uploadIcon) {
                                    uploadIcon.className = "fa-solid fa-spinner fa-spin upload-icon text-indigo-600";
                                }
                                btnText.textContent = "Качване...";
                                input.disabled = true;
                                ...
                                fetch('/admin/storage/ajax-upload', {
                                    ...
                                    .then(data => {
                                        // Променено от data.path на data.url съобразно твоя JSON response
                                        if (data.success && data.url) {
                                            ...
                                            if (zoomBtn) {
                                                zoomBtn.setAttribute('data-src', uploadedUrl);
                                            }

                                            const downloadBtn = container.querySelector('a[id^="download-btn-"]');
                                            if (downloadBtn) {
                                                downloadBtn.setAttribute('href', uploadedUrl);
                                            }

                                            actions.classList.remove('hidden');
                                            overlay.classList.remove('hidden');

                                            // Тук също записваме пълния URL в скрития input
                                            removeInput.value = uploadedUrl;
                                            btnText.textContent = "Промяна";
                                        } else {
                                            alert(data.error || 'Възникна грешка при качването.');
                                            btnText.textContent = removeInput.value ? "Промяна" : "Качване";
                                        }
                                    })
                                    .catch(error => {
                                    ...
                                    .finally(() => {
                                        if (uploadIcon) {
                                            uploadIcon.className = "fa-solid fa-plus upload-icon";
                                        }
                    ...
                    document.body.addEventListener('click', function (e) {
                        ...
                        if (removeBtn && !removeBtn.classList.contains('lightbox-trigger') && removeBtn.tagName !== 'A') {
    ...
    public static function multiImage(string $label, string $name, array $currentImages = [])
    {
        ...
        <div class="space-y-3" data-component="multi-image-upload" id="<?= $id ?>">
            ...
            <div
                class="drop-zone flex flex-col items-center justify-center p-8 bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200 transition-all hover:bg-slate-50/80 hover:border-primary/50 cursor-pointer relative group">
                <input type="file" multiple accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer file-input">
                <div class="text-center pointer-events-none">
                    <i
                        class="fa-solid fa-cloud-arrow-up text-3xl text-slate-400 mb-2 group-hover:text-primary transition-colors"></i>
                    <div class="md:font-bold text-sm text-slate-700">Влачете изображения тук или <span
                            class="text-primary hover:underline">кликнете за избор</span></div>
                    <div class="text-xs text-slate-400 mt-1">Поддържат се всички модерни формати за изображения: JPG, PNG, WebP
                        (Макс. 10MB)</div>
                </div>
            </div>
            ...
            <div class="previews-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 2xl:grid-cols-5 gap-4 mt-4">
                <?php foreach ($currentImages as $path): ?>
                    <div class="relative group aspect-video rounded-xl overflow-hidden shadow-sm border border-slate-200 bg-white image-item"
                        ...
                        <div
                            class="absolute inset-0 bg-black/40 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-300 pointer-events-none lg:pointer-events-auto">
                        </div>

                        <div
                            class="absolute top-2 right-2 z-10 flex gap-1.5 p-1 rounded-lg bg-black/20 lg:bg-transparent backdrop-blur-sm lg:backdrop-blur-none">
                            <button type="button"
                                class="btn-view-img w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center bg-white text-slate-700 rounded-md shadow-md hover:bg-primary hover:text-white transition-all transform hover:scale-110"
                                title="Преглед на голям екран">
                                <i class="fa-solid fa-eye text-xs sm:text-sm"></i>
                            </button>
                            <button type="button"
                                class="btn-delete-img w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center bg-white text-red-500 rounded-md shadow-md hover:bg-red-500 hover:text-white transition-all transform hover:scale-110"
                                title="Премахване">
            ...
            <div
                class="lightbox-modal fixed inset-0 z-100 hidden bg-black/80 backdrop-blur-sm items-center justify-center p-4 opacity-0 transition-opacity duration-200">
                <button type="button"
                    class="lightbox-close absolute top-5 right-5 text-white/70 hover:text-white text-3xl p-2 transition-colors cursor-pointer"
                    title="Затваряне (Esc)">
                    <i class="fa-solid fa-xmark"></i>
                </button>
                <div class="max-w-5xl max-h-[85vh] relative flex items-center justify-center pointer-events-none">
                    <img class="lightbox-img max-w-full max-h-[85vh] rounded-lg shadow-2xl border border-white/10 object-contain pointer-events-auto"
                        src="" alt="Преглед">
                </div>
            </div>
        </div>
    ...
    public static function mainSubmit($label = 'Запазване', $icon = 'fa-floppy-disk')
    {
        ...
        <button type="button" data-role="main-submit-btn"
