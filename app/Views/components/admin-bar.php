            ...
            <div class="container mx-auto flex items-center justify-between px-4 min-w-max gap-8">
                ...
                <div class="flex items-center gap-6 shrink-0 h-full">
                    <a href="/admin/dashboard" class="flex items-center gap-2 transition-colors group whitespace-nowrap">
                        ...
                        <span
                            class="text-xs font-semibold uppercase text-gray-500 hover:text-white duration-300">Табло</span>
                    </a>
                </div>
                    ...
                    <div class="flex items-center gap-6 border-l border-slate-800 pl-6 h-full shrink-0">
                        <a href="/admin/users/edit/<?= AuthHelper::user()['id'] ?>"
                            ...
                            <div class="hidden sm:flex flex-col items-end">
                                <span
                                    class="text-[9px] text-slate-500 uppercase tracking-tighter leading-none mb-1 group-hover/user:text-primary transition-colors">
                                    Администратор
                                </span>
                                <span
                                    class="text-xs text-slate-200 font-bold leading-none border-b border-transparent group-hover/user:border-primary/30">
                                    <?= $user['name'] ?>
                                </span>
                            </div>
                            ...
                            <?php else: ?>
                                <div
                                    class="w-8 h-8 bg-slate-800 border border-white/10 rounded-lg flex items-center justify-center shadow-inner group-hover/user:border-primary/50 group-hover/user:bg-primary/10 transition-all">
                                    <span
                                        class="text-[10px] font-semibold text-slate-400 group-hover/user:text-primary uppercase">
                        ...
                        <form action="/users/logout" method="POST" class="m-0 p-0 flex shrink-0">
                            <button type="submit"
                                ...
                                <span>Изход</span>
                                <i
                                    class="fa-solid fa-power-off text-xs opacity-70 group-hover:rotate-90 transition-transform duration-300"></i>
                            </button>
                        </form>
                    </div>
                </div>
