...
class IncomingCallModule : Module() {
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("startOngoingCall") { options: IncomingCallDisplayOptions ->
      val context = context()
      val isVideo = options.callType != "audio"
      if (!hasMediaPermissions(context, isVideo)) {
        Log.w(TAG, "[CALL] ongoing service skipped: media permission is missing")
      } else {
        try {
          OngoingCallService.start(
            context,
            options.callerName.ifBlank { "Обаждане в ход" },
            if (isVideo) "Видео обаждане" else "Аудио обаждане",
            isVideo,
          )
  ...
  private fun hasMediaPermissions(context: Context, isVideo: Boolean): Boolean {
    val hasAudio = ContextCompat.checkSelfPermission(
      context,
      Manifest.permission.RECORD_AUDIO,
    ) == PackageManager.PERMISSION_GRANTED
    val hasCamera = !isVideo || ContextCompat.checkSelfPermission(
      context,
