...
class OngoingCallService : Service() {
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    ...
    // Android 14+ rejects startForeground() when the runtime permissions for
    // the requested service types are missing. Do not let that exception kill
    // the application; the JS call flow will continue without the ongoing
    // call notification until permissions are granted.
    if (!hasPermission(Manifest.permission.RECORD_AUDIO) ||
      (isVideo && !hasPermission(Manifest.permission.CAMERA))) {
      android.util.Log.w(TAG, "Skipping ongoing call service: media permission is missing")
      stopSelf()
